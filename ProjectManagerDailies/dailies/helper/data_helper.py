...
def get_all_contractors_info_for_report(cursor, report, report_number):
    contractor_report = {}
    contractor_report['contractors'] = {}
    query = 'SELECT DISTINCT EQ.ctr_code_id, C.company_name FROM dailies_contractor C, dailies_equipment EQ, \
     dailies_equipment_report EQR WHERE EQR.report_id = ' + str(report_number) + ' AND EQR.equipment_id = EQ.id AND \
     EQ.ctr_code_id = C.ctr_code'
    ...
    for result in results:
        dictname = str(i)
        contractor_report['contractors'][dictname] = {}
        contractor_report['contractors'][dictname]['ctr_code'] = result['ctr_code_id']
        contractor_report['contractors'][dictname]['company_name'] = result['company_name']

        i += 1
    report.update(contractor_report)

    return report

def get_daily_report_info(report_number):
    """
    Notes:  Connects and Discconects to database and returns a dictionary of daily report data
    Args:   cursor (from Connection()), report_number (the id of the report you want)
    Return: report dictionary with all necessary info
    """
    (connection, cursor) = connect('server.config')
    ...
    report = get_all_contractors_info_for_report(cursor, report, report_number)
    # report = json.dumps(report) #This get it into JSON type
    disconnect(connection, cursor)
    return report

# Testings the variables
# print(get_daily_report_info(1))
